...
from ..cpd import BaseCPD, CPD, NNCPD, RandomisedCPD, Parafac2
...
class TestNNCPD:
    ...
    def test_init(self):
        ...
        cpd = NNCPD(init=init,
                  ...
                  verbose=verbose)
        assert not cpd.cost  # check that this list is empty
        assert cpd.name == NNCPD.__name__
        assert cpd.init == init
        assert cpd.max_iter == max_iter
        assert cpd.epsilon == epsilon
        assert cpd.tol == tol
    ...
    def test_init_fmat(self):
        ...
        # and there would be a runtime warning
        rank = (min(tensor.shape) + 1,)
        cpd = NNCPD(init='svd', verbose=True)
        with pytest.warns(RuntimeWarning):
            fmat = cpd._init_fmat(tensor=tensor, rank=rank)
        for mat in fmat:
            result_1 = np.dot(mat.T, mat)
    ...
    def test_decompose(self):
        ...
        cpd = NNCPD(init='random', max_iter=50, epsilon=10e-3, tol=10e-5)
